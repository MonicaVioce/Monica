...
export async function POST(request) {
  ...
  const statusUrl = `${base}/api/stream-status${token ? `?token=${encodeURIComponent(token)}` : ''}`;
  const xml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Connect>
    <Stream url="${xmlEscape(streamUrl)}" track="inbound_track" codec="PCMU" bidirectionalMode="rtp" bidirectionalCodec="PCMU" bidirectionalSamplingRate="8000" statusCallback="${xmlEscape(statusUrl)}" />
  </Connect>
</Response>`;
