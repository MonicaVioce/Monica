...
export function validateCaseContext(value) {
  ...
  const permitted = ['customer_name', 'customer_phone', 'company', 'reservation_or_case_id', 'issue', 'requested_resolution', 'acceptance_limit', 'authorized_actions'];
  const result = {};
  ...
  if (result.customer_phone && !/^\+[1-9]\d{6,14}$/.test(result.customer_phone)) throw new Error('customer_phone must be E.164, e.g. +14045551234.');
  return result;
}
